...
{
    /// <summary>
    /// Public class to log Error/info messages to the log file
    /// </summary>
    public sealed class NLogger : ITraceWriter
    {
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (record.Request != null)
            {
                ...
                if (record.Request.Headers != null && record.Request.Headers.Contains("SessionId") && record.Request.Headers.GetValues("SessionId") != null && record.Request.Headers.GetValues("SessionId").FirstOrDefault() != null)
                    message.Append("").Append("SessionId: " + record.Request.Headers.GetValues("SessionId").FirstOrDefault() + Environment.NewLine);
                //if (record.Request.Headers != null && record.Request.Headers.Contains("X-Request-ID") && record.Request.Headers.GetValues("X-Request-ID") != null && record.Request.Headers.GetValues("X-Request-ID").FirstOrDefault() != null)
                //    message.Append("").Append("X-Request-ID: " + record.Request.Headers.GetValues("X-Request-ID").FirstOrDefault() + Environment.NewLine);
                //    //message.Append("").Append("Headers: " + record.Request.Headers.ToString() + Environment.NewLine);
            }            
            ...
            if (record.Exception != null && !string.IsNullOrWhiteSpace(record.Exception.GetBaseException().Message))
            {
                ...
                //    }
                //}
                //else if (exceptionType == typeof(ErrorList<ApiBusinessException>))
                //{
                //    var validationErrors = record.Exception as ErrorList<ApiBusinessException>;
                //    if (validationErrors.Errors != null && validationErrors.Errors.Any())
                //    {
                //        message.Append("").Append("Error: ");
                //        foreach (var error in validationErrors.Errors)
                //        {
                //            message.Append(error.ErrorDescription + Environment.NewLine);
                //        }
                //    }
                //}
                if (typeof(IResponseSummaryException).IsAssignableFrom(exceptionType))
                {
                    var ex = record.Exception as IResponseSummaryException;
                    if (ex.Messages != null && ex.Messages.Any())
                    {
                        message.Append("").Append("Error: ");
                        foreach (var error in ex.Messages)
                        {
                            message.Append(error.Description + Environment.NewLine);
                        }
                    }
                }
                else
            ...
            //moved the json to the end so that error gets logged even if request JSON gets truncated due to DB table/proc size limit
